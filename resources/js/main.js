...
const threeProject = (() => {
	...
	let seahorse, seahorseGeometry, seahorseParticles, seahorseFloatingGeometry, seahorseFlying, seahorseFloatingParticles, seahorseBody, seahorseColors;
	let whaleVertices = [];
	...
	const setTheModel = () => {
		...
		gltfLoader.load("../resources/models/seahorse.glb", (object) => {
			...
			seahorseFloatingGeometry = new THREE.BufferGeometry();

			seahorseParticles = new Float32Array(seahorseVertices.length * particleCount);
			seahorseFloatingParticles = new Float32Array((seahorseVertices.length * particleCount) / 2);

			for (let i = 0; i < 4; i++) {
				for (let j = 0; j < seahorseVertices.length; j++) {
					seahorseParticles[j + seahorseVertices.length * i] = seahorseVertices[j] + Math.random() * 0.2;
					seahorseFloatingParticles[j + seahorseVertices.length * i] = seahorseVertices[j] + Math.random() * 0.25;
				}
			}
		});
	...
	const setTheParticle = () => {
		...
		gsap.to(whaleFlying.geometry.attributes.position.array, {
			endArray: seahorseFloatingParticles,
			ease: "power2",
			scrollTrigger: {
				trigger: whaleContainer,
				start: "top top",
